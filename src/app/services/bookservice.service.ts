...
export class BookserviceService {
  books: Book[] = [
    ...
    },
    {
      ...
      img_src:"assets/advancedjava.JPG",
      file_path:"",
      category:"learn",
      author:"ANDRIY REDKO",
      isinCart:false,
  ...
  getbook(id:number){
    return this.books.filter((data) => data.id == id)[0];
  }
  delete(id: number) {
  ...
  addToCart(book:Book){
    this.books.filter((data)=> data== book)[0].isinCart=true
    this.cartService.add(book);
  }
  removeToCart(book:Book){
    this.books.filter((data)=> data== book)[0].isinCart=false
